...
class BidirectionalAStarPlanner:
    ...
    def __init__(self, ox, oy, oz, resolution, rr):
        ...
        self.x_width, self.y_width, self.z_width, self.obstacle_map = None, None, None, None
    ...
    def planning(self, sx, sy, sz, gx, gy, gz):
        ...
        while 1:
            ...
            if current_A.x == current_B.x and current_A.y == current_B.y and current_A.z == current_B.z:
                print("Found goal")
            ...
            for i, _ in enumerate(self.motion):
                ...
                c_nodes = [self.Node(current_A.x + self.motion[i][0],
                                     ...
                                     current_A.z + self.motion[i][2],
                                     current_A.cost + self.motion[i][3],
                                     c_id_A),
                           self.Node(current_B.x + self.motion[i][0],
                                     current_B.y + self.motion[i][1],
                                     current_B.z + self.motion[i][2],
                                     current_B.cost + self.motion[i][3],
                                     c_id_B)]
        ...
        rx, ry, rz = self.calc_final_bidirectional_path(
    ...
    def calc_final_bidirectional_path(self, n1, n2, setA, setB):
        rx_A, ry_A, rz_A = self.calc_final_path(n1, setA)
        rx_B, ry_B, rz_B = self.calc_final_path(n2, setB)
    ...
    def calc_final_path(self, goal_node, closed_set):
        ...
        rx, ry, rz = [self.calc_grid_position(goal_node.x, self.min_x)], [self.calc_grid_position(goal_node.y, self.min_y)], [self.calc_grid_position(goal_node.z, self.min_z)]
    ...
    def calc_grid_index(self, node):
        return (node.z - self.min_z) * self.y_width + (node.y - self.min_y) * self.x_width + (node.x - self.min_x)
    ...
    def calc_obstacle_map(self, ox, oy, oz):
        ...
        self.obstacle_map = [[[False for _ in range(self.y_width)] for _ in range(self.x_width)] for _ in range(self.z_width)]
            ...
            for iy in range(self.y_width):
                ...
                for iz in range(self.z_width):
                    ...
                    for iox, ioy, ioz in zip(ox, oy, oz):
                        ...
                        if d <= self.rr:
                            self.obstacle_map[iz][iy][ix] = True
                            break
...
def main():
    ...
    bidir_a_star = BidirectionalAStarPlanner(ox, oy, oz, grid_size, robot_radius)
    rx, ry, rz = bidir_a_star.planning(sx, sy, sz, gx, gy, gz)
