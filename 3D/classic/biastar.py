...
from helper.utils import dist_euclidiana, distancia_rota3D, diminuir_pontos, simulate_points, pseudo3D
    ...
    def planning(self, sx, sy, gx, gy, show, vmx, vmy):
        ...
        while 1:
            if len(open_set_A) == 0 or len(open_set_B) == 0:
                _, _, rotax, rotay, _ = ajudante.run(show=False, vmx=vmx, vmy=vmy, startx=sx, starty=sy)
                return rotax, rotay
...
def run(show=False, vmx=None, vmy=None, startx=None, starty=None, p1=None):
    ...
    a, b = diminuir_pontos(pathx, pathy, p.xobs, p.yobs, apf=1)
    print(a)
    ...
    # return distancia, end, a, b, [2]*len(a)
